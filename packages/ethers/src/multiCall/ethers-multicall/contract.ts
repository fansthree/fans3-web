...
export class Contract {
  ...
  constructor(address: string, abi: JsonFragment[] | string[] | Fragment[]) {
    ...
    const callFunctions = this._functions.filter((x) => ['pure', 'view', 'nonpayable'].includes(x.stateMutability))
...
function makeCallFunction(contract: Contract, name: string) {
  return (...params: any[]) => {
    ...
    const { inputs } = <any>contract.functions.find((f) => f.name === name)
    const { outputs } = <any>contract.functions.find((f) => f.name === name)
    return {
      contract: {
        address
      },
